...
linux_plugin_docker:
	go mod vendor -v
	docker run --rm -it -v "$PWD":/filebeat-throttle-plugin golang:1.10.6 go build -v -o /output/filebeat_throttle_linux.so -buildmode=plugin github.com/ozonru/filebeat-throttle-plugin/register/plugin
